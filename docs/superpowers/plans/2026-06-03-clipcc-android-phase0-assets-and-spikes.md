...
> The HF-documented expectation is `tokenizer_json` (the v5 SigLIP2 normalizer includes
> `Lowercase`). If the spike prints `kotlin_wrapper`, that is a **red flag** that the shipped
> artifact differs from source — record it prominently in the spike report and confirm the
> full-pipeline `parity_ok` is still `True` before proceeding.
